...
export const GetAdminStats = async (req: Request, res: Response) => {
  try {
    ...
    // Post Office Stats
    const pendingPostOfficeApplications = await Users.countDocuments({
        postOfficeStatus: "pending",
        role: { $ne: "admin" }
    });

    const acceptedPostOffice = await Users.countDocuments({
         role: "post_office" 
    });

    const rejectedPostOfficeApplications = await Users.countDocuments({
        postOfficeStatus: "rejected"
    });
    ...
    // Calculate MRR (Monthly Recurring Revenue - Revenue from last 30 days)
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    
    const mrrResult = await Payments.aggregate([
        { 
            $match: { 
                status: "paid",
    ...
    const stats = {
      ...
      pendingVendorApplications,
      pendingPostOfficeApplications,
      acceptedPostOffice,
      rejectedApplications,
      rejectedPostOfficeApplications,
      reportedItems,
    };
...
export const GetUsersByRole = async (req: Request, res: Response) => {
  try {
    ...
    const allowedRoles = ["user", "vendor", "post_office", "post_office_member"];
