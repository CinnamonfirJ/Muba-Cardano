...
const StoreForm = ({ storeId }: StoreFormProps) => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        setIsLoading(true);
        // Get current user
        // Priority: useAuth user, then authService
        ...
        if (isEditMode && storeId) {
          await loadStoreData(storeId, user);
        }
      } catch (err: any) {
    ...
    loadInitialData();
  }, [storeId, isEditMode, authUser]);

  const loadStoreData = async (id: string, user: any) => {
    try {
      setError(null);
      ...
      if (store.owner) {
        ...
        // Allow loading if we can't verify permissions? Better strict.
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let response;
      if (isEditMode && storeId) {
        response = await storeService.updateStore(storeId, formDataPayload as any);
      } else {
